use crate::share::incomplete_vector_from_share_response;
use crate::share::responses::ListSharesResponse;
...
#[derive(Debug, Clone)]
pub struct ListSharesBuilder<'a> {
...
impl<'a> ListSharesBuilder<'a> {
    ...
    pub async fn execute(
        &self,
    ) -> Result<ListSharesResponse, Box<dyn std::error::Error + Sync + Send>> {
        let mut url = self
            .storage_client
        ...
        if let Some(include) = match (self.include_metadata, self.include_deleted, self.include_snapshot) {
            (true, true, false) => Some("metadata,deleted"),
            (true, false, false) => Some("metadata"),
            (false, true, false) => Some("deleted"),
            (false, false, false) => None,
            (true, true, true) => Some("metadata,deleted,snapshot"),
            (true, false, true) => Some("metadata,snapshot"),
            (false, true, true) => Some("deleted,snapshot"),
            (false, false, true) => Some("snapshot"),
        } {
        ...
        let incomplete_vector = incomplete_vector_from_share_response(&body)?;
    ...
    pub fn stream(
        self,
    ) -> impl Stream<Item = Result<ListSharesResponse, Box<dyn std::error::Error + Sync + Send>>> + 'a
    {
