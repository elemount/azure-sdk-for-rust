...
use azure_core::headers::{ REQUEST_ID, SNAPSHOT };
...
impl TryFrom<(&Bytes, &HeaderMap)> for SnapshotResponse {
    type Error = crate::Error;

    fn try_from((body, header_map): (&Bytes, &HeaderMap)) -> Result<Self, Self::Error> {
        SnapshotResponse::from_response(body, header_map)
    }
}
...
impl SnapshotResponse {
    ...
    pub(crate) fn from_response(
        body: &Bytes,
        headers: &HeaderMap,
    ) -> Result<SnapshotResponse, crate::Error> {
        ...
        let snapshot = match headers.get(SNAPSHOT) {
            Some(snapshot) => snapshot.to_str()?,
            None => return Err(crate::Error::MissingHeaderError(SNAPSHOT.to_owned())),
        };
